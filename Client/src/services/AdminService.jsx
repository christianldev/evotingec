		...
		axios.post(API + '/election', data).then((r) => {
			if (r.status === 200) {
				...
				ws.getContract().then((c) => {
					ws.getCurrentAccount().then((a) => {
						c.methods
							.addElection(election.electionId)
							.send({from: a})
							.on('confirmation', (d) => {
								console.log(d);
								resolve(d);
		...
		getAllElections().then((r) => {
			...
			getAllConstituencies().then((c) => {
				...
				elections.forEach((e, i) => {
					...
					ec.name = constituencies.filter(
						(c, i) => c.id === e.constituencyId
					)[0].name;
					ecs.push(ec);
...
export const getCandidateVote = (eId) => {
	return new Promise((resolve, reject) => {
		getAllCandidatesServer().then(async (r) => {
			console.log(r);
			let candidates = await r.data.filter(
			...
			candidates.forEach((ca, i) => {
				ws.getContract().then((c) => {
					console.log(ca.id);
					c.methods
						.getCandidateVotes(ca.id.toString())
						.call()
						.then((result) => {
